...
def chat_generate_kpm(tok, model, system_msg: str, user_msg: str, max_new_tokens: int = MAX_NEW_TOKENS):
    ...
    out_tokens = tok.encode(text, add_special_tokens=False)
    return {
...
def main():
    ...
    # 동시 접근 시 OOM 방지용 모델별 락 (한 모델당 1개 generate 동시 진행)
    lock_q1 = threading.Lock()
    lock_q2 = threading.Lock()

    def _one_call(which: str):
        if which == "Q1":
            with lock_q1:
    ...
    def run_round(n_concurrent: int):
        # [MOD] n=1일 때 Q1+Q2를 같은 라운드에서 함께 실행
        if n_concurrent == 1:
            tags = ["Q1", "Q2"]                 # 한 라운드에 Q1, Q2 둘 다
            max_workers = 2                     # 둘을 동시에 돌릴 수 있게 2
        else:
            # 기존 동작 유지: Q1/Q2 번갈아 n_concurrent개 생성
            tags = [("Q1" if i % 2 == 0 else "Q2") for i in range(n_concurrent)]
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as ex:
            futs = [ex.submit(_one_call, tag) for tag in tags]
        ...
        earliest_first = min(r["t_first"] for r in results)
        latest_done = max(r["t_done"] for r in results)
        window = max(1e-9, latest_done - earliest_first)
        ...
        total_throughput = total_tokens / window

        # 결과 출력
        print(f"[ROUND {n_concurrent}]")
        print(f"Requests_in_round: {len(results)}")  # [MOD] 실제 요청 수 표시
