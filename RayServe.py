...
def run_serve_benchmark(user_q1: str, user_q2: str, sys_msg: str):
    ...
    q1_app = ServeModelWorker.options(
        name="q1_worker",                       # ← 문자열 필수
        num_replicas=n_q1,
        ray_actor_options={"num_gpus": 1},
    ).bind(model_dir=str(Q1_PATH))
    serve.run(q1_app, name="q1_app", route_prefix=None)           # 앱별로 개별 run. :contentReference[oaicite:2]{index=2}
    ...
    # 핸들 취득(DeploymentHandle): .result()로 회수. :contentReference[oaicite:3]{index=3}
    ...
    for n in ROUNDS:
        ...
        results = [r.result() for r in futures]   # ← ray.get(...) 금지(Serve 응답). :contentReference[oaicite:4]{index=4}
